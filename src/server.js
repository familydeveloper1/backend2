...
// GÜVENLİ BÖLGE ROTALARI

// Tüm güvenli bölgeleri getir
app.get('/api/safe-zones', auth, async (req, res) => {
  try {
    const userId = req.user._id;
...
app.get('/api/safe-zones/phone/:phoneNumber', auth, async (req, res) => {
  try {
    const userId = req.user._id;
    const { phoneNumber } = req.params;
...
app.post('/api/safe-zones', auth, async (req, res) => {
  try {
    const userId = req.user._id;
...
app.put('/api/safe-zones/:id', auth, async (req, res) => {
  try {
    const userId = req.user._id;
    const { id } = req.params;
...
// Güvenli bölgeyi sil
app.delete('/api/safe-zones/:id', auth, async (req, res) => {
  try {
    const userId = req.user._id;
    const { id } = req.params;
...
app.post('/api/safe-zones/check-location', auth, async (req, res) => {
  try {
    const userId = req.user._id;
...
app.get('/api/safe-zones/:id/events', auth, async (req, res) => {
  try {
    const userId = req.user._id;
    const { id } = req.params;
